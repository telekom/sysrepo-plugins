 ...
 *
 * @param session Session to use for retreiving/set datastore data.
 */
void UserValuesApplier::applyDatastoreValues(sysrepo::Session& session)
{
    ...
    session.deleteItem("/ietf-system:system/authentication");
    ...
    users.loadFromSystem();
    for (auto& user : users) {
        user.AuthorizedKeys = std::make_optional<ietf::sys::auth::AuthorizedKeyList>();
        user.AuthorizedKeys->loadFromSystem(user.Name);
    }

    for (auto& user : users) {

        session.setItem("/ietf-system:system/authentication/user[name='" + user.Name + "']", std::nullopt);
        if (user.Password) {
            session.setItem("/ietf-system:system/authentication/user[name='" + user.Name + "']/password", user.Password);
        }

        if (user.AuthorizedKeys) {
            for (auto& key : user.AuthorizedKeys.value()) {
                session.setItem("/ietf-system:system/authentication/user[name='" + user.Name + "']/authorized-key[name='" + key.Name + "']/algorithm", key.Algorithm);
                session.setItem("/ietf-system:system/authentication/user[name='" + user.Name + "']/authorized-key[name='" + key.Name + "']/key-data", key.Data);
